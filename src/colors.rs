...
//! Ultralytics color palettes and the `Color` type.
//!
//! Kept in a wasm-safe module (not under the native-only `visualizer`) so the
//! browser bindings can reuse the exact same palette as the native annotator,
//! making colors consistent across the Rust, Python, and browser paths.
